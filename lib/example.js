...
var FlicBattery = fliclib.FlicBatteryStatusListener;

var client = new FlicClient("192.168.1.2" , 5551);

function listenToButton(bdAddr) {
	var cc = new FlicConnectionChannel(bdAddr);
	var bat = new FlicBattery(bdAddr)
	client.addConnectionChannel(cc);
	client.addBatteryStatusListener(bat)
	//let info = client.getInfo(cc)
	//console.log(info)
	cc.on("data", function(clickType, wasQueued, timeDiff) {
		console.log(bdAddr + " " + clickType + " " + (wasQueued ? "wasQueued" : "notQueued") + " " + timeDiff + " seconds ago");
	});
	bat.on("batteryStatus", function( percentage, timestamp){
		console.log(percentage, timestamp )
	});
	

}

//info = client.getInfo("80:e4:da:76:89:89")
//console.log(info)
